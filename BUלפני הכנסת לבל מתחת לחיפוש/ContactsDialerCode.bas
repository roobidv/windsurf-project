...
Public Sub SetupContactsDialerForm()
    ...
    titles = Array("ttlPhone", "ttlLandline", "ttlEmail", "ttlDateAdded", "ttlNotes", "ttlHistory", "btnImportExcel")
    ...
    ' ===== btnImportExcel (import contacts from Excel) =====
    Err.Clear: stepName = "btnImportExcel"
    Set ctl = CreateControl(frmName, acCommandButton, CLng(sec), "", "", margin, curTop, colW, 480)
    If Err.Number = 0 Then
        ctl.Name = "btnImportExcel"
        ctl.Caption = "ייבוא אנשי קשר מקובץ Excel"
        ctl.FontSize = 11: ctl.FontBold = True: ctl.FontName = "Segoe UI"
        ctl.ForeColor = RGB(255, 255, 255)
        ctl.BackColor = clrAccent: ctl.UseTheme = False
        ctl.OnClick = "=ContactsDialer_BtnImportExcel_Click()"
    End If: Err.Clear
    curTop = curTop + 480 + gap
    If sec = 1 Then hTop = curTop Else dTop = curTop
...
Private Sub EnsureCallHistoryGridForm()
    ...
    ' Add bound TextBox controls — 5 columns
    ...
    Set ctl = Application.CreateControl(tmpName, acTextBox, acDetail, "", "CallTime", 3400, 0, 1200, 300)
    ctl.Name = "txtCallTime": ctl.Format = "hh:nn:ss"
    Set ctl = Application.CreateControl(tmpName, acTextBox, acDetail, "", "Notes", 4700, 0, 2400, 300)
    ctl.Name = "txtNotes"
    Set ctl = Application.CreateControl(tmpName, acTextBox, acDetail, "", "ContactName", 7200, 0, 2000, 300)
    ctl.Name = "txtContactName"
