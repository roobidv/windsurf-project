...
Public Function ContactsDialer_Form_Load() As Variant
    ...
    frm.TimerInterval = 100  ' 100ms for Ctrl+~ hotkey responsiveness
...
Private Sub DialSpeedNumber(ByRef frm As Access.Form, ByVal phoneNum As String, ByVal speedName As String)
    ...
    If scContactId > 0 Then
        ...
        Set rsScExt = CurrentDb.OpenRecordset("SELECT Extension FROM tblSettings WHERE ComputerName='" & Environ("COMPUTERNAME") & "'", dbOpenSnapshot)
...
Public Sub LoadSpeedCallButtons(ByRef frm As Access.Form)
    ...
    Set rs = CurrentDb.OpenRecordset("SELECT * FROM tblSettings WHERE ComputerName=""" & Environ("COMPUTERNAME") & """", dbOpenSnapshot)
    If rs.EOF Then
        rs.Close
        ...
        If Not rs.EOF Then
            ...
            If Len(msgText) > 0 Then
                ...
                If Len(pcName) > 0 Then
                    CurrentDb.Execute "UPDATE tblSettings SET MSG = NULL WHERE ComputerName=""" & pcName & """", dbFailOnError
                End If
            Else
                rs.Close: Set rs = Nothing
            End If
...
Private Sub LogAppOpen()
    ...
    Set rsEmp = db.OpenRecordset("SELECT EmployeeName FROM tblSettings WHERE ComputerName=""" & Environ("COMPUTERNAME") & """", dbOpenSnapshot)
...
Private Function GetMyExtension() As String
    On Error Resume Next
    Dim rs As DAO.Recordset
    Set rs = CurrentDb.OpenRecordset("SELECT Extension FROM tblSettings WHERE ComputerName='" & Environ("COMPUTERNAME") & "'", dbOpenSnapshot)
