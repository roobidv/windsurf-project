...
Public Function ContactEdit_BtnSave_Click() As Variant
    ...
    If Len(contactId) > 0 And contactId <> "0" Then
        ...
        Application.Run "UpdateRow", "Contacts", phoneNumber, "ContactName", contactName, "FamlyName", famlyName, "Tital", tital, "Landline", landline, "Email", email, "Notes", notes
        On Error GoTo ErrorHandler
    Else
        ' --- INSERT ---
        ...
        ' --- Sync to Google ---
        On Error Resume Next
        If targetTable = "tblGLOBAL_PHONE_BOOK" Then
            Application.Run "PushPhoneBookEntry", contactName, famlyName, tital, landline, phoneNumber, email, "", notes
        Else
            Application.Run "PushContact", contactName, famlyName, tital, phoneNumber, landline, email, "", notes
        End If
        On Error GoTo ErrorHandler
        ' --- ????? ????? ????? ---
        On Error Resume Next
        Application.Run "LogEvent", "NewContact", contactName & " " & phoneNumber
        On Error GoTo ErrorHandler
    End If
...
' קבלת פוקוס בשדה טקסט - סימון כל הטקסט בשדה לעריכה נוחה
...
' איבוד פוקוס משדה טקסט - ניקוי סימון בעזיבת השדה
