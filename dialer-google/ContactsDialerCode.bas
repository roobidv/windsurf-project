...
' ===========================================================================
' מודול ראשי: ContactsDialerCode
' תיאור: לוגיקה לטופס הראשי frmContactsDialer
' יכולות:
'   - חיפוש אנשי קשר (txtSearch)
'   - תצוגת פרטי איש קשר + טלפונים
'   - היסטוריית שיחות (sfrmCallHistory)
'   - חיוג מהיר (btnSpeed1..18)
'   - ייבוא מ-Excel
'   - פקודות מנהל (#גיבוי#, #שחזר#, #לוג#, #שדרוג#)
' טבלאות: Contacts, CallHistory, tblSettings
' ===========================================================================
...
Private Const APP_VERSION As String = "1.0.0"  ' גרסת אפליקציה
...
Public Function ContactsDialer_Form_Load() As Variant
    ...
    ' כפתורי טלפון מתחילים מנוטרלים עד שנבחר איש קשר
    frm.cmdPhoneNumber.Enabled = False
    frm.cmdPhoneNumber.caption = ChrW$(1500) & ChrW$(1488) & " " & ChrW$(1511) & ChrW$(1497) & ChrW$(1497) & ChrW$(1501) & " " & ChrW$(1502) & ChrW$(1505) & ChrW$(1508) & ChrW$(1512)  ' לא קיים מספר
    frm.cmdLandline.Enabled = False
    frm.cmdLandline.caption = ChrW$(1500) & ChrW$(1488) & " " & ChrW$(1511) & ChrW$(1497) & ChrW$(1497) & ChrW$(1501) & " " & ChrW$(1502) & ChrW$(1505) & ChrW$(1508) & ChrW$(1512)  ' לא קיים מספר
    ' כפתורי עריכה/שליחה/תיעוד מושבתים עד שנבחר איש קשר
    ...
    frm.lstContacts.ControlTipText = ChrW$(1504) & ChrW$(1497) & ChrW$(1493) & ChrW$(1493) & ChrW$(1496) & " " & ChrW$(1502) & ChrW$(1506) & ChrW$(1500) & ChrW$(1492) & " " & ChrW$(1502) & ChrW$(1496) & ChrW$(1492) & " " & ChrW$(1506) & ChrW$(1501) & " " & ChrW$(1499) & ChrW$(1508) & ChrW$(1514) & ChrW$(1493) & ChrW$(1512) & ChrW$(1497) & " " & ChrW$(1492) & ChrW$(1495) & ChrW$(1497) & ChrW$(1510) & ChrW$(1497) & ChrW$(1501)  ' ניווט מעלה מטה עם כפתורי החיצים
    frm.txtSearch.ControlTipText = ChrW$(1499) & ChrW$(1491) & ChrW$(1497) & " " & ChrW$(1500) & ChrW$(1495) & ChrW$(1494) & ChrW$(1493) & ChrW$(1512) & " " & ChrW$(1500) & ChrW$(1514) & ChrW$(1488) & " " & ChrW$(1495) & ChrW$(1497) & ChrW$(1508) & ChrW$(1493) & ChrW$(1513) & " " & ChrW$(1504) & ChrW$(1511) & ChrW$(1497) & " " & ChrW$(1500) & ChrW$(1495) & ChrW$(1509) & vbCrLf & "ESC"  ' כדי לחזור לתא חיפוש נקי לחץ  (שורה חדשה)  ESC
    frm.btnNewCus.ControlTipText = "Ctrl+N"
    frm.btnEditCus.ControlTipText = "Ctrl+E"
    frm.lblNoteNow.ControlTipText = ChrW$(1492) & ChrW$(1511) & ChrW$(1500) & ChrW$(1511) & " " & ChrW$(1508) & ChrW$(1506) & ChrW$(1502) & ChrW$(1497) & ChrW$(1497) & ChrW$(1501) & " " & ChrW$(1500) & ChrW$(1506) & ChrW$(1512) & ChrW$(1497) & ChrW$(1499) & ChrW$(1514) & " " & ChrW$(1513) & ChrW$(1497) & ChrW$(1495) & ChrW$(1492) & " " & ChrW$(1504) & ChrW$(1489) & ChrW$(1495) & ChrW$(1512) & ChrW$(1514)   ' הקלק פעמיים לעריכת שיחה נבחרת
    frm.txtSearch.SetFocus                          ' פוקוס התחלתי על תיבת חיפוש
    On Error Resume Next
    Application.Run "StartHotkey"                     ' Ctrl+` גלובלי להחזרת פוקוס ל-Access
    On Error GoTo 0
...
' ---------------------------------------------------------------------------
' הפעלה מחדש של הטופס - מחזיר פוקוס לתיבת חיפוש ומעביר מקלדת לעברית
' נקרא כשהטופס מקבל פוקוס (כולל אחרי מזעור/שחזור)
...
Public Function ContactsDialer_CmdPhoneNumber_Click() As Variant
    ...
    ' נותנים פוקוס על ידי חיפוש כותרת 3CX
    DialerGetFocus
    Sleep 300
...
' ---------------------------------------------------------------------------
' InsertCallHistoryRecord: מוסיף רשומה חדשה לטבלת CallHistory ומרענן את הגריד
' נקרא מתוך CmdPhoneNumber_Click ו-CmdLandline_Click
' ---------------------------------------------------------------------------
Private Sub InsertCallHistoryRecord(ByRef frm As Access.Form, ByVal phoneNumber As String)
    ...
    ' שליפת שלוחה מ-tblSettings לפי שם מחשב
    ...
    ' רענון גריד שיחות (דגל כדי שה-OnCurrent לא ידרוס את ההודעה)
    m_skipGridNoteUpdate = True
    ContactsDialer_RefreshCallHistoryGrid frm, contactId, True
    ' הנחיה למשתמש לתעד את השיחה + רקע צהוב
    frm.lblNoteNow.caption = ChrW$(1500) & ChrW$(1514) & ChrW$(1497) & ChrW$(1506) & ChrW$(1493) & ChrW$(1491) & " " & ChrW$(1492) & ChrW$(1513) & ChrW$(1497) & ChrW$(1495) & ChrW$(1492) & " " & ChrW$(1492) & ChrW$(1511) & ChrW$(1500) & ChrW$(1511) & " " & ChrW$(1508) & ChrW$(1506) & ChrW$(1502) & ChrW$(1497) & ChrW$(1497) & ChrW$(1501) & " " & ChrW$(1499) & ChrW$(1488) & ChrW$(1503)   ' לתיעוד השיחה הקלק פעמיים כאן
...
' ---------------------------------------------------------------------------
' ---------------------------------------------------------------------------
' btnNewMail Click: פתיחת מייל חדש והעברת הסמן לשורת הנושא בצד ימין
' ---------------------------------------------------------------------------
Public Function ContactsDialer_BtnNewMail_Click() As Variant
    ...
    If Len(email) > 0 Then
        ...
        ' שלב 3: וידוא סופי שהסמן בימין (בלי לגרום לשגיאה)
        DoEvents
        
    Else
        MsgBox "לא נמצאה כתובת אימייל.", vbInformation, "שליחת דואר"
    End If
...
ErrorHandler:
    MsgBox "חלה שגיאה: " & Err.Description, vbCritical
...
' ---------------------------------------------------------------------------
' קבלת פוקוס בתיבת חיפוש - מעביר מקלדת לעברית
...
' ---------------------------------------------------------------------------
' קבלת פוקוס בשדה אימייל - מעביר מקלדת לאנגלית
...
' ---------------------------------------------------------------------------
' btnExit Click: שחרור כל המשאבים וסגירת האפליקציה
' משחרר Recordsets, סוגר טופס גריד, סוגר טופס ראשי, יוצא מ-Access.
' btnExit property: On Click = =ContactsDialer_BtnExit_Click()
' ---------------------------------------------------------------------------
Public Function ContactsDialer_BtnExit_Click() As Variant
    On Error Resume Next

    ' 1. סגירת Recordset אנשי קשר
    If Not m_rsContacts Is Nothing Then
        m_rsContacts.Close
        Set m_rsContacts = Nothing
    End If

    ' 2. סגירת Recordset היסטוריית שיחות
    If Not m_rsCallHistory Is Nothing Then
        m_rsCallHistory.Close
        Set m_rsCallHistory = Nothing
    End If

    ' 3. ניתוק Recordset מגריד (אם קיים)
    Dim frm As Access.Form
    Set frm = Forms("frmContactsDialer")
...
' ---------------------------------------------------------------------------
' btnNewCus Click: פתיחת טופס עריכה לרשומה חדשה
...
' ---------------------------------------------------------------------------
' btnEditCus Click: פתיחת טופס עריכה לרשומה הנבחרת
...
Public Function ContactsDialer_BtnEditCus_Click() As Variant
    ...
    If Len(contactId) = 0 Or contactId = "0" Then
        MsgBox ChrW$(1489) & ChrW$(1495) & ChrW$(1512) & " " & ChrW$(1488) & ChrW$(1497) & ChrW$(1513) & " " & ChrW$(1511) & ChrW$(1513) & ChrW$(1512) & " " & ChrW$(1502) & ChrW$(1492) & ChrW$(1512) & ChrW$(1513) & ChrW$(1497) & ChrW$(1502) & ChrW$(1492), _
               vbInformation, "frmContactsDialer"   ' בחר איש קשר מהרשימה
        GoTo Done
    End If

    DoCmd.OpenForm "frmContactEdit", acNormal, , , , acDialog, contactId
    ' אחרי סגירת הדיאלוג – רענון רשימת אנשי קשר
    ContactsDialer_RefreshAfterEdit
Done:
...
' ---------------------------------------------------------------------------
' cmdSetings Click: פתיחת טופס הגדרות
...
' ---------------------------------------------------------------------------
' btnSendCus Click: העברת פוקוס לאפליקציית 3CX
...
' ---------------------------------------------------------------------------
' רענון רשימה וגריד אחרי הוספה/עריכה של רשומה
' ---------------------------------------------------------------------------
...
' ---------------------------------------------------------------------------
' btnImportVCF Click: ייבוא כרטיס VCF - פתיחת דיאלוג בחירת קובץ
' ---------------------------------------------------------------------------
        ...
        If Len(searchVal) >= 3 And Left$(searchVal, 1) = "#" And Right$(searchVal, 1) = "#" Then
            ...
            Select Case cmdText
                Case "גיבוי", "גבוי"
                    ...
                    frm.txtSearch.Value = ""
                    MsgBox "גיבוי בוצע!", vbInformation, "גיבוי"
                Case "שחזר", "שיחזור"
                    frm.txtSearch.Value = ""
                ...
                Case "לוג"
                    ...
                    ViewErrorLog
                Case Else
                    MsgBox "פקודת מנהל לא קיימת", vbExclamation
                    frm.txtSearch.Value = ""
            End Select
            ContactsDialer_Form_KeyDown = True
            Exit Function
...
' ---------------------------------------------------------------------------
' חיוג מהיר - מבצע חיוג למספר שמאוחסן בכפתור חיוג מקוצר
' מתעד שיחה ב-CallHistory, מעתיק ללוח גזירים, ומחייג דרך 3CX
...
' ---------------------------------------------------------------------------
' לחיצה על כפתור חיוג מקוצר (btnSpeed1..18) - מחלץ מספר מ-Tag ומחייג
...
' ---------------------------------------------------------------------------
' טעינת כפתורי חיוג מקוצר - קורא שמות ומספרים מ-tblSettings ומציב בכפתורים
' נקרא מ-Form_Load ואחרי שמירת הגדרות
Public Sub LoadSpeedCallButtons(ByRef frm As Access.Form)
    On Error Resume Next
    Dim rs As DAO.Recordset
    ' קריאה לפי שם מחשב, אם אין התאמה - רשומה ראשונה
...
NextSpeedBtn:
    ...
    If Not rs Is Nothing Then
        If Not rs.EOF Then
            Dim msgText As String
            msgText = Nz(rs.Fields("MSG"), "")
            If Len(msgText) > 0 Then
                Dim pcName As String
                pcName = Nz(rs.Fields("ComputerName"), "")
                rs.Close: Set rs = Nothing
                MsgBox msgText, vbInformation, "הודעה מרובי דביר"
                ' מחיקת הודעה אחרי לחיצת OK
                If Len(pcName) > 0 Then
                    CurrentDb.Execute "UPDATE tblSettings SET MSG = NULL", dbFailOnError
                End If
            Else
                rs.Close: Set rs = Nothing
            End If
...
' ---------------------------------------------------------------------------
' CreateSpeedCallButtons - ONE-TIME: creates 18 buttons inside BoxSpeedCall
' Run from Immediate window:  CreateSpeedCallButtons
' ---------------------------------------------------------------------------
' יצירה חד-פעמית של 18 כפתורי חיוג מקוצר בתוך BoxSpeedCall
' להרצה מ-Immediate Window בלבד
Public Sub CreateSpeedCallButtons()
    On Error GoTo ErrHandler
    ...
    ' --- Read btnSpeed2 dimensions as template ---
    Dim refBtn As Control
    Set refBtn = frm.Controls("btnSpeed2")
    Dim btnH As Long, btnW As Long
    btnH = refBtn.Height
    btnW = refBtn.Width
    Debug.Print "Template btnSpeed2: W=" & btnW & " H=" & btnH

    Dim i As Long, ctl As Control
    Dim boxL As Long, boxT As Long, boxW As Long, boxSection As Long
    Dim marginL As Long, marginT As Long, btnGap As Long

    ' --- btnSpeed1..9 in BoxSpeedCall ---
    boxL = frm.Controls("BoxSpeedCall").Left
    boxT = frm.Controls("BoxSpeedCall").Top
    boxW = frm.Controls("BoxSpeedCall").Width
    boxSection = frm.Controls("BoxSpeedCall").Section
    Dim boxH As Long
    boxH = frm.Controls("BoxSpeedCall").Height
    btnGap = (boxH - 40 - 9 * btnH) \ 8
    If btnGap < 10 Then btnGap = 10
    marginL = boxL + (boxW - btnW) \ 2
    Dim totalH As Long
    totalH = 9 * btnH + 8 * btnGap
    marginT = boxT + (boxH - totalH) \ 2
    If marginT < boxT + 10 Then marginT = boxT + 10
    For i = 1 To 9
        If i <> 2 Then
            On Error Resume Next
            DeleteControl "frmContactsDialer", "btnSpeed" & i
            On Error GoTo ErrHandler
        End If
        Set ctl = Nothing
        If i = 2 Then
            Set ctl = frm.Controls("btnSpeed2")
        Else
            Set ctl = CreateControl("frmContactsDialer", acCommandButton, CLng(boxSection), "", "", _
                marginL, _
                marginT + (i - 1) * (btnH + btnGap), _
                btnW, _
                btnH)
            ctl.Name = "btnSpeed" & i
        End If
        ctl.Left = marginL
        ctl.Top = marginT + (i - 1) * (btnH + btnGap)
        ctl.Width = btnW
        ctl.Height = btnH
        ctl.Caption = i & ":"
    ...
    MsgBox "18 Speed Call buttons created (sized like btnSpeed2)!", vbInformation
    Exit Sub
ErrHandler:
    MsgBox "CreateSpeedCallButtons Error: " & Err.Number & " - " & Err.Description, vbCritical
End Sub
...
' ---------------------------------------------------------------------------
' בדיקת אבחון - מדפיסה את מצב כפתורי החיוג המקוצר ל-Immediate Window
...
' ---------------------------------------------------------------------------
' תיקון טופס בפתיחה - מוודא שכל אירועי הטופס מחוברים נכון
...
' ---------------------------------------------------------------------------
' LogAppOpen - רישום פתיחת אפליקציה ב-tblAppLog
' ---------------------------------------------------------------------------
Private Sub LogAppOpen()
    On Error Resume Next
    Dim db As DAO.Database
    Set db = CurrentDb
    ' חיפוש שם עובד לפי שם מחשב ב-tblSettings
...
' ---------------------------------------------------------------------------
' LogAppClose - עדכון שעת סגירה ב-tblAppLog
' ---------------------------------------------------------------------------
Private Sub LogAppClose()
    On Error Resume Next
    If m_logID = 0 Then Exit Sub
    ' כתיבה ישירה ל-BE כדי להבטיח שהעדכון יתבצע לפני Application.Quit
...
' ייצוא איש קשר נוכחי ל-Outlook ומציג לפני שמירה
' ---------------------------------------------------------------------------
Public Function ContactsDialer_BtnExport2Outlook_Click() As Variant
    ...
    If IsNull(frm.lblContactID.Value) Or frm.lblContactID.Value = "" Then
        MsgBox "לא נבחר איש קשר", vbExclamation, "Outlook"
        GoTo Done
    End If
    ...
    If rs.NoMatch Then
        MsgBox "איש קשר לא נמצא", vbExclamation, "Outlook"
        GoTo Done
    End If
...
ErrorHandler:
    MsgBox "שגיאה בייצוא ל-Outlook: " & Err.Description, vbExclamation, "Outlook"
    ContactsDialer_BtnExport2Outlook_Click = True
End Function

' ---------------------------------------------------------------------------
' GetMyExtension - שליפת שלוחה מ-tblSettings לפי שם מחשב
' ---------------------------------------------------------------------------
' ---------------------------------------------------------------------------
' HideAccessFrame - הסתרת מסגרת Access, הצגת טופס צף בלבד
' קריאה מ-Form_Load
' ---------------------------------------------------------------------------
' ---------------------------------------------------------------------------
...
' AdjustFormWidth - +5%
' ---------------------------------------------------------------------------
' התאמת רוחב טופס - מכוונן רוחב חלון לפי הפקדים
...
' CenterChildForm
' ---------------------------------------------------------------------------
' מרכוז טופס-בן - מציב חלון פופ-אפ במרכז המסך
...
' הסתרת מסגרת Access - מסתיר את החלון הראשי ב-ACCDE
...
' ---------------------------------------------------------------------------
' ShowAccessFrame - הצגת מסגרת Access בחזרה
' קריאה מ-Form_Unload
' ---------------------------------------------------------------------------
...
' ---------------------------------------------------------------------------
' SetupAccdeProperties - הגדרות להפעלה עם ACCDE
' הרצה פעם אחת מ-Immediate
' ---------------------------------------------------------------------------
Public Sub SetupAccdeProperties()
    ...
    MsgBox "הגדרות ACCDE נקבעו", vbInformation, "Setup"
End Sub

' הגדרת מאפיין מסד נתונים - כותב ערך למאפייני ה-DB
...
' קבלת שלוחה - מחזיר את מספר השלוחה מ-tblSettings
...
' הקמת טופס חייגן - יוצר ומעצב את כל הפקדים בטופס frmContactsDialer
' אזהרה: מוחק עיצוב ידני! להרצה חד-פעמית בלבד
...
Public Sub StyleContactsDialerForm()
    ...
    If Not btnExp Is Nothing Then
        btnExp.caption = "ייצוא ל-Outlook"
...
' מגדיר את frmContactsDialer כטופס שנפתח אוטומטית כשהמסד נפתח.
' הרצה חד-פעמית מ-Immediate:  SetDialerAsStartupForm
' ---------------------------------------------------------------------------
Public Sub SetDialerAsStartupForm()
    On Error Resume Next
    Dim db As DAO.Database
    Set db = CurrentDb
    ' ניסיון לעדכן Property קיים
    db.Properties("StartupForm") = "frmContactsDialer"
    If Err.number <> 0 Then
        ' Property לא קיים – יוצר חדש
        Err.Clear
        Dim prp As DAO.Property
...
Private Sub ContactsDialer_LoadSelectedContact(ByRef frm As Access.Form, ByVal contactId As Long)
    ...
    ' איפוש כפתורי עריכה/שליחה/תיעוד כשיש איש קשר נבחר
...
Private Sub ContactsDialer_ClearDisplay()
    ...
    ' כפתורי עריכה/שליחה/תיעוד מושבתים כשאין בחירה
    frm.btnEditCus.Enabled = False
    frm.btnSendCus.Enabled = False
    frm.btnSaveCall.Enabled = False
...
Private Sub ContactsDialer_RefreshCallHistoryGrid(ByRef frm As Access.Form, ByVal contactId As Long, Optional ByVal newestFirst As Boolean = False)
    ...
    Dim sql As String
    If newestFirst Then
        ' לאחר שיחה יוצאת – מיון לפי CallID יורד כדי שהגריד יעמוד על הרשומה האחרונה
        sql = "SELECT H.CallID, H.PhoneNumber, H.CallDate, H.CallTime, H.Notes, C.ContactName " & _
...
' כשהמשתמש עובר בין שורות בגריד – מעדכן את הטולטיפ עם תוכן שדה ההערה.
...
Public Function ContactsDialer_Grid_Current() As Variant
    ...
    ' הצגת ההערה בשורת המצב (Status Bar) בתחתית חלון Access
    If Len(notes) > 0 Then
        SysCmd acSysCmdSetStatus, ChrW$(1492) & ChrW$(1506) & ChrW$(1512) & ChrW$(1492) & ": " & notes   ' הערה:
    Else
        SysCmd acSysCmdSetStatus, " "
    End If
    ' הצגת הערה בתווית lblNoteNow (דילוג אם הדגל פעיל)
    If m_skipGridNoteUpdate Then
        m_skipGridNoteUpdate = False
    Else
        frm.lblNoteNow.caption = notes
        frm.lblNoteNow.BackStyle = 0   ' Transparent (ביטול רקע צהוב)
    End If
Done:
...
' ---------------------------------------------------------------------------
' lblNoteNow DblClick: פותח טופס עריכת רשומת שיחה עבור השורה הנוכחית בגריד
...
' ---------------------------------------------------------------------------
' יצירת תפריט קליק ימני להעתקה - תפריט קיצור לכפתורי טלפון
...
' ---------------------------------------------------------------------------
' ביצוע העתקה מתפריט קליק ימני - מעתיק טקסט ללוח גזירים
...
' ---------------------------------------------------------------------------
' הצגת תפריט העתקה - מציג פופ-אפ עם אפשרות העתקה ללוח גזירים
...
' קליק ימני על רשימת אנשי קשר - מציג תפריט העתקה
...
' קליק ימני על כפתור נייד - מציג תפריט העתקת מספר
...
' קליק ימני על כפתור נייח - מציג תפריט העתקת מספר
...
' קליק ימני על שדה הערות - מציג תפריט העתקה
...
' קליק ימני על שדה אימייל - מציג תפריט העתקת כתובת מייל
...
' קליק ימני על תווית חיפוש - מציג תפריט העתקת שם
...
' ---------------------------------------------------------------------------
' העתקה ללוח גזירים - מעתיק טקסט ל-Clipboard באמצעות Windows API
...
' ---------------------------------------------------------------------------
' ייבוא איש קשר מקובץ VCF - מפענח כרטיס ביקור ומכניס לטבלת Contacts
